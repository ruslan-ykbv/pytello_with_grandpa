...
class DroneManager(metaclass=Singleton):
    def __init__(self, host_ip='192.168.10.2', host_port=8889,
        ...
        self.dronie_event = None
        self.is_dronie = False
        self._dronie_semaphore = threading.Semaphore(1)
        self._thread_dronie = None
    ...
    def dronie(self):
        if not self.is_dronie:
            self.dronie_event = threading.Event()
            self._thread_dronie = threading.Thread(
                target=self._dronie,
                args=(self._dronie_semaphore, self.dronie_event,))
            self._thread_dronie.start()
            self.is_dronie = True

    def stop_dronie(self):
        if self.is_dronie:
            self.dronie_event.set()
            retry = 0
            while self._thread_dronie.isAlive():
                time.sleep(0.3)
                if retry > 300:
                    break
                retry += 1
    ...
    def _dronie(self, semaphore, stop_event):
        is_acquire = semaphore.acquire(blocking=False)
        speed = self.speed
        if self.is_patrol:
            self.stop_patrol()
        if is_acquire:
            logger.info({'action': '_dronie', 'status': 'acquire'})
            with contextlib.ExitStack() as stack:
                stack.callback(semaphore.release)
                status = 0
                while not stop_event.is_set():
                    status += 1
                    if status == 1:
                        self.send_command(f'go -35 30 35 {speed}',
                                          blocking=False)
                    if status == 2:
                        self.clockwise(15)
                    if status == 3:
                        self.snapshot()
                    if status == 4:
                        self.counter_clockwise(15)
                    if status == 5:
                        self.send_command(f'go 0 -30 -35 {speed}',
                                          blocking=False)
                    if status == 6:
                        self.snapshot()
                    if status == 7:
            ...
            logger.warning({'action': '_dronie', 'status': 'not_acquire'})
